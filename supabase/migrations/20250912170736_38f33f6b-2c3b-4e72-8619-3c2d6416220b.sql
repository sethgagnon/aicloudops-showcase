-- Security Enhancement: Ultra-secure audit log policies
...
BEGIN
    ...
    -- Log super admin access attempts for security monitoring
    IF user_has_access THEN
        INSERT INTO public.contact_security_audit (
            user_id, action, table_name, success, created_at
        ) VALUES (
            current_user_id, 'super_admin_audit_access', 'security_logs', true, now()
        );
    ELSE
        ...
        INSERT INTO public.contact_security_audit (
            ...
            current_user_id, 'unauthorized_audit_access', 'security_logs', false, 
            'Insufficient privileges for audit log access', now()
        );
    END IF;
...
ON public.contact_access_logs 
FOR INSERT 
WITH CHECK (true);  -- System functions need to insert
...
BEGIN
    ...
    INSERT INTO public.contact_security_audit (
        ...
        auth.uid(), 'super_admin_promotion', 'user_roles', true, now()
    );
...
CREATE INDEX IF NOT EXISTS idx_contact_access_logs_created_at ON public.contact_access_logs(created_at DESC);
...
-- Create a policy for the profile_access_audit table we created earlier
CREATE POLICY "Super admins only: view profile audit" 
